#include "TChannelMusicHistograms.hxx"
...
void THistoCharges::CreateHistograms()
{
  ...
  {
      int size = sizeof(maxChargeHisto);
      status = db_get_value(hDB, 0, "/Equipment/V1730_Data00/Analysis/MaxChargeHisto", &maxChargeHisto, &size, TID_INT, TRUE);
  }

  // check if we already have histogramss and maxChargeHisto Has Changed.
  if(size() != 0)
  {
    char tname[100];
    ...
    if (tmp)
    {
      if (fMaxChargeHisto!=maxChargeHisto)
      { 
        fMaxChargeHisto=maxChargeHisto;
        delete tmp;
      }
      else return;
    }
  }
  std::cout << "*************** MAX CHARGE FOR HISTO: " << fMaxChargeHisto << std::endl;
  ...
  { // loop over 16 channels
      ...
      // MARIA TODO
      TH1D *tmp = new TH1D(name, title, 2000, 0,fMaxChargeHisto);
      //TH1D *tmp = new TH1D(name, title, 1000, 0,4e6);
...
{
  TMusicEvent * dev = TEventProcessor::instance()->GetDartEvent();
...
{
  char name[100];
  sprintf(name,"ChargeSummaryMusic");
  ...
  int maxCharge= 50000 ;
  char title[100];
  sprintf(title,"Music Charge Summary");
  TH1D *tmp = new TH1D(name, title, 2000, 0,maxCharge);
  tmp->SetXTitle("Charge (ADC*sample value)");
  tmp->SetYTitle("Counts");
  push_back(tmp);

  sprintf(title,"Veto Charge Summary");
  tmp = new TH1D(name, title, 2000, 0,maxCharge);
  tmp->SetXTitle("Charge (ADC*sample value)");
  tmp->SetYTitle("Counts");
  push_back(tmp);
}
