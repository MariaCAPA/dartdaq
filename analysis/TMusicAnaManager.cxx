#include "TMusicAnaManager.hxx"
#include "TChannelHistograms.hxx"
#include <fstream>

TMusicAnaManager::TMusicAnaManager()
{
...
void TMusicAnaManager::AddHistogram(THistogramArrayBase* histo) {
...
void TMusicAnaManager::BeginRun(int transition,int run,int time, MVOdb* odb)
{
  TEventProcessor::instance()->BeginRun(run);

  // MARIA print chage histogram
  for (int i=0; i<fHistos.size(); i++)
  {

    THistogramArrayBase* harray = fHistos[i];
    int num = harray->size();
    for (int j=0; j<num; j++)
    {
      TH1F * h = (TH1F*)((*harray)[j]);
      h->Reset();
    }
  }
...
int TMusicAnaManager::ProcessMidasEvent(TDataContainer& dataContainer)
{

  TEventProcessor::instance()->ProcessMidasEvent(dataContainer);  // MusicEvent accesible for all histos
...
void TMusicAnaManager::UpdateTransientPlots(TDataContainer& dataContainer)
{
...
void TMusicAnaManager::EndRun(int transition,int run,int time) 
{
  TEventProcessor::instance()->EndRun(run);

  // MARIA print chage histogram 
  /*
  if (fHistos.size()>1)
  {
    THistogramArrayBase* harray = fHistos[1];
    int num = harray->size();
    for (int j=0; j<num; j++)
    {
      TH1F * h = (TH1F*)((*harray)[j]);
        int n=h->GetNbinsX();
        std::fstream fq (Form("histo%d_%d.txt",run,j),std::ios::out);
        for (int i=0; i<n; i++) fq << h->GetBinContent(i) << std::endl;
        fq.close();
        std::cout << " ********** file " << Form("histo%d_%d.txt",run,j) << " saved  "<< std::endl;
      h->Reset();
    }
  }
  else std::cout << " no charge histo " << std::endl;
  */
}
